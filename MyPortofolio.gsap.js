...
left.forEach((left, i) => {
  const animLeft = gsap.fromTo(left, {autoAlpha: 0, x: -50}, {duration: 1, autoAlpha: 1, x: 0});
...
right.forEach((right, i) => {
  const animRight = gsap.fromTo(right, {autoAlpha: 0, x: 50}, {duration: 1, autoAlpha: 1, x: 0});
  ScrollTrigger.create({
    trigger: right,
    animation: animRight,
    toggleActions: 'restart restart restart restart',
